...
-(NSMutableArray *)selectDataByCategory:(int)dr
{
    ...
    NSMutableArray *hdataArray=[[NSMutableArray alloc] init];
    NSMutableArray *wdataArray=[[NSMutableArray alloc] init];
    sqlite3 *database;
    ...
    if(result==SQLITE_OK){
        while(sqlite3_step(statement)==SQLITE_ROW){
            ...
            if ([hdataArray indexOfObject:languageItem.hanyu_info]==NSNotFound&&[wdataArray indexOfObject:languageItem.weiyu_info]) {
                [hdataArray addObject:languageItem.hanyu_info];
                [wdataArray addObject:languageItem.weiyu_info];
...
-(NSMutableArray *)selectData:(NSString *)str{
    ...
    NSString *query=[NSString stringWithFormat:@"SELECT * FROM 'userful_data' where weiyu_info  LIKE '%%%@%%' or hanyu_info LIKE '%%%@%%' ORDER BY hanyu_info ASC",str,str];
    sqlite3_stmt *statement;
    int result=sqlite3_prepare_v2(database, [query UTF8String], -1, &statement, nil);
    if(result==SQLITE_OK){
        while(sqlite3_step(statement)==SQLITE_ROW){
